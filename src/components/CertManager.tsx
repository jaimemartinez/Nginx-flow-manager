...
export const CertManager: React.FC<CertManagerProps> = ({ open, onClose }) => {
  ...
  const runIssue = async () => {
    ...
    if (list.length === 0) { setOutput('Indica al menos un dominio.'); setOutputOk(false); return; }
    setBusy(true);
    setOutput('Ejecutando certbot... esto puede tardar.');
    setOutputOk(null);
    try {
  ...
  const runRenew = async (dryRun: boolean) => {
    setBusy(true);
    setOutput(`Ejecutando certbot renew${dryRun ? ' --dry-run' : ''}...`);
    setOutputOk(null);
    try {
  ...
  const confirmIssue = () => {
    const list = domains.split(/[\s,]+/).map(d => d.trim()).filter(Boolean);
    askConfirmation(
      staging ? 'Emitir certificado (STAGING/prueba)' : 'Emitir certificado REAL',
      `Se ejecutará certbot en el servidor para: ${list.join(', ') || '(sin dominios)'}.\n` +
      (staging
        ? 'Modo staging: certificado de PRUEBA (no válido en navegadores, sin coste de rate-limit). Úsalo primero para validar.'
        : '⚠️ Modo REAL: cuenta contra el rate-limit de Let\'s Encrypt (5 certs/dominio/semana). Asegúrate de que el DNS apunta a este servidor.'),
      () => { runIssue(); }
    );
  };
      ...
      >
        ...
        <div className="bg-[#0A0A0B] border-b border-white/10 px-5 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-white font-display">Certificados SSL · Let's Encrypt</h2>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={fetchCerts} className="p-1.5 text-slate-400 hover:text-white rounded hover:bg-white/5 cursor-pointer" title="Refrescar">
        ...
        <div className="overflow-y-auto p-5 space-y-5">
          {!installed && (
            <div className="bg-amber-500/5 border border-amber-500/20 rounded p-3 flex items-center gap-2 text-[12px] text-amber-300 font-mono">
              <AlertTriangle size={15} className="shrink-0" /> certbot no está instalado en el servidor. Instálalo (apt install certbot python3-certbot-nginx) para gestionar certificados.
            </div>
          )}

          {/* Certificate list */}
          <div>
            <span className="block text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Certificados instalados</span>
            {certs.length === 0 ? (
              <div className="text-[12px] text-slate-500 italic py-3">{loading ? 'Cargando...' : 'No hay certificados (o certbot no disponible).'}</div>
            ) : (
            ...
            <div className="flex gap-2 mt-2">
              <button
                onClick={() => askConfirmation('Probar renovación', 'Ejecuta "certbot renew --dry-run" (no modifica nada, solo prueba).', () => runRenew(true))}
                disabled={busy}
              ...
              >
                <RefreshCw size={12} /> Probar renovación (dry-run)
              </button>
              <button
                onClick={() => askConfirmation('Renovar certificados', 'Ejecuta "certbot renew" REAL en el servidor. Renueva los que caduquen pronto.', () => runRenew(false))}
                disabled={busy}
          ...
          <div className="border-t border-white/10 pt-4">
            <span className="block text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Emitir nuevo certificado</span>
            <div className="space-y-2.5">
              <div>
                <label className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Dominios (separados por coma o espacio)</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Email (opcional)</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Método</label>
                  <select
                  ...
                  >
                    <option value="webroot">webroot (sirve el challenge desde un dir)</option>
                    <option value="nginx">nginx (plugin reescribe la config)</option>
                  </select>
                </div>
              </div>
              {method === 'webroot' && (
              ...
              >
                ...
                Emitir certificado{staging ? ' (staging)' : ''}
              </button>
            </div>
          </div>
