...
export const AgentPanel: React.FC<AgentPanelProps> = ({ open, onClose }) => {
  ...
  const install = async (endpoint: string) => {
    setBusy(true); setSteps([]); setMsg('Instalando y verificando el agente en el servidor...');
    try {
      const r = await secureFetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
      const d = await r.json();
      setSteps(d.steps || []);
      if (d.skipped) setMsg('No aplica (no estás en modo remoto).');
      else if ((d.installed || d.alreadyInstalled || d.success) && d.reachable) setMsg('Agente instalado y verificado up & running ✓');
      else if (d.installed || d.alreadyInstalled || d.success) setMsg('Agente instalado pero no responde aún: ' + (d.error || ''));
      else setMsg('Falló la instalación: ' + (d.error || ''));
      await fetchStatus();
  ...
  const uninstall = async () => {
    ...
    setBusy(true); setSteps([]); setMsg('Desinstalando el agente y revirtiendo los cambios...');
    try {
      const r = await secureFetch('/api/agent/uninstall', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
      const d = await r.json();
      setSteps(d.steps || []);
      setMsg(d.success ? 'Agente desinstalado y cambios revertidos ✓' : 'Desinstalación incompleta: ' + (d.error || ''));
      await fetchStatus();
    } catch (e: any) { setMsg('Error: ' + e.message); }
    finally { setBusy(false); }
  };
      ...
      <div className="bg-[#121214] border border-white/10 rounded-lg shadow-2xl w-full max-w-2xl max-h-[88vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="bg-[#0A0A0B] border-b border-white/10 px-5 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-white font-display">Agente seguro del servidor</h2>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="overflow-y-auto p-5 space-y-4">
          ...
          <div className={`rounded-lg p-4 border ${!installed ? 'bg-white/[0.02] border-white/10' : reachable ? 'bg-[#009639]/5 border-[#009639]/25' : 'bg-amber-500/5 border-amber-500/25'}`}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white">
                {!installed ? 'Agente no instalado' : reachable ? 'Up & running ✓' : 'Instalado pero no responde'}
              </span>
            </div>
            {installed && status?.info && (
              <div className="mt-2 text-[10px] font-mono text-slate-400 space-y-0.5">
                ...
                <div>agente: <span className="text-slate-300">v{status.info.agent}</span></div>
              </div>
            )}
            {installed && !reachable && status?.error && (
              <p className="mt-1 text-[10px] text-amber-300/80 font-mono">{status.error}</p>
            )}
          </div>
          ...
          <div className="text-[10px] text-slate-500 leading-relaxed flex gap-2">
            ...
            <span>El agente no abre ningún puerto: solo se accede por SSH con una clave restringida a un único comando (forced command) y un HMAC por petición. Sustituye la ejecución de shell crudo por una API acotada con deploy atómico y rollback.</span>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {installed ? 'Verificar / reparar agente' : 'Instalar agente seguro'}
            </button>
            {installed && (
              <button
