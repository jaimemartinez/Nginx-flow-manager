      ...
      <div className="w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col bg-[#0A0A0B] border border-white/10 rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <div className="flex items-center gap-2 text-slate-200 font-mono font-bold text-sm">
            <Users size={16} className="text-[#009639]" /> Gestión de usuarios
          </div>
        ...
        <div className="overflow-y-auto px-4 py-3 space-y-2">
          {loading ? <div className="text-slate-500 text-xs font-mono py-4 text-center">Cargando…</div> : users.map((u) => (
            <div key={u.id} className="flex items-center gap-2 bg-white/[0.03] border border-white/5 rounded px-3 py-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-slate-200 text-xs font-mono font-bold truncate">
                  {u.username}{u.username === currentUsername && <span className="ml-1.5 text-[9px] text-slate-500">(tú)</span>}
                </div>
                <div className="text-[9px] text-slate-600 font-mono">{new Date(u.createdAt).toLocaleString()}</div>
                {resetId === u.id && (
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <input type="password" autoFocus value={resetPw} onChange={(e) => setResetPw(e.target.value)} placeholder="nueva contraseña (≥8)"
                      className="flex-1 bg-[#0A0A0B] border border-white/10 rounded px-2 py-1 text-[10px] text-slate-200 font-mono" />
                    <button disabled={busy === u.id} onClick={() => resetPassword(u)} className="text-[9px] font-bold uppercase bg-[#009639] hover:bg-[#00b347] text-white px-2 py-1 rounded cursor-pointer disabled:opacity-50">Guardar</button>
                    <button onClick={() => { setResetId(null); setResetPw(''); }} className="text-[9px] text-slate-500 hover:text-slate-300 px-1 cursor-pointer">Cancelar</button>
                  </div>
                )}
              </div>
              <select value={u.role} disabled={busy === u.id} onChange={(e) => changeRole(u, e.target.value as NfmRole)}
                className={`text-[10px] font-mono font-bold uppercase rounded px-1.5 py-1 border cursor-pointer ${roleClass[u.role]}`} title={ROLE_DESC[u.role]}>
                {ROLES.slice().reverse().map((r) => <option key={r} value={r} className="bg-[#0A0A0B] text-slate-200">{ROLE_LABEL[r]}</option>)}
              </select>
              <button onClick={() => { setResetId(resetId === u.id ? null : u.id); setResetPw(''); }} title="Cambiar contraseña"
                className="p-1.5 text-slate-500 hover:text-amber-300 rounded cursor-pointer"><KeyRound size={13} /></button>
              <button onClick={() => deleteUser(u)} disabled={busy === u.id} title="Eliminar usuario"
        ...
        <div className="border-t border-white/10 px-4 py-3">
          <div className="text-[9px] text-slate-500 uppercase font-bold font-mono mb-1.5">Nuevo usuario</div>
          <div className="flex flex-wrap items-center gap-1.5">
            <input value={nu} onChange={(e) => setNu(e.target.value)} placeholder="usuario"
              className="flex-1 min-w-[120px] bg-[#0A0A0B] border border-white/10 rounded px-2 py-1.5 text-[11px] text-slate-200 font-mono" />
            <input type="password" value={np} onChange={(e) => setNp(e.target.value)} placeholder="contraseña (≥8)"
              className="flex-1 min-w-[120px] bg-[#0A0A0B] border border-white/10 rounded px-2 py-1.5 text-[11px] text-slate-200 font-mono" />
            <select value={nr} onChange={(e) => setNr(e.target.value as NfmRole)}
              ...
              {ROLES.slice().reverse().map((r) => <option key={r} value={r}>{ROLE_LABEL[r]}</option>)}
            </select>
          ...
          <div className="text-[9px] text-slate-600 mt-1.5">{ROLE_DESC[nr]}</div>
        </div>
      </div>
    </div>,
    document.body,
