...
/**
 * Sanitizes root-level custom directives to prevent importing standard system template commands or comments.
 */
function sanitizeMainCustomDirectives(directives: string): string {
  if (!directives) return '';
  const lines = directives.split('\n');
  const ignorePhrases = [
    "load copied nginx modules",
    "lua script & caching support",
    "maestro nginx",
    "file: /etc/nginx",
    "generated by nginx",
    "custom global root directives",
    "modules-enabled",
    "applet_id",
    "disable_auth_bridge",
    "worker_processes",
  ...
  const filtered = lines.filter(line => {
    ...
    // Ignore matching pattern comments or phrases
    for (const phrase of ignorePhrases) {
      if (lower.includes(phrase)) return false;
    }
    
    // Ignore system directives
    for (const dir of ignoreDirectives) {
...
/**
 * Sanitizes HTTP-level custom directives to prevent importing standard system template commands or comments.
 */
function sanitizeHttpCustomDirectives(directives: string): string {
  if (!directives) return '';
  const lines = directives.split('\n');
  const ignorePhrases = [
    "mime mappings",
  ...
  const filtered = lines.filter(line => {
    ...
    // Ignore whole server block if it was somehow appended
    if (lower.startsWith('server') && (lower.endsWith('{') || lower.includes('{'))) {
      return false;
    }
    if (trimmed === '}') {
      return false;
